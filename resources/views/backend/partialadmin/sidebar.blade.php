
    <div class="main-menu menu-fixed menu-light menu-accordion    menu-shadow " data-scroll-to-active="true">
        <div class="main-menu-content">
            <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
                <li class="active"><a href="index.html"><i class="la la-home"></i><span class="menu-title" data-i18n="Dashboard Hospital">Dashboard Hospital</span></a>
                </li>
                <li class=" navigation-header"><span data-i18n="Professional">Professional</span><i class="la la-ellipsis-h" data-toggle="tooltip" data-placement="right" data-original-title="Professional"></i>
                </li>
                <li class=" nav-item"><a href="#"><i class="la la-edit"></i><span class="menu-title" data-i18n="Appointment">Appointment</span></a>
                    <ul class="menu-content">
                        <li><a class="menu-item" href="hospital-doctor-schedule.html"><i></i><span>Jadwal Appoinment</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-book-appointment.html"><i></i><span>Buat Appointment</span></a>
                        </li>
                    </ul>
                </li>
                <li class=" nav-item"><a href="#"><i class="la la-stethoscope"></i><span class="menu-title" data-i18n="Doctors">Konselor</span></a>
                    <ul class="menu-content">
                        <li><a class="menu-item" href="hospital-doctors-list.html"><i></i><span>Daftar Konselor</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-add-doctor.html"><i></i><span>Tambah Konselor</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-doctor-profile.html"><i></i><span>Profil Konselor</span></a>
                        </li>
                    </ul>
                </li>
                <li class=" nav-item"><a href="#"><i class="la la-users"></i><span class="menu-title" data-i18n="Patients">Mahasiswa</span></a>
                    <ul class="menu-content">
                        <li><a class="menu-item" href="hospital-patients-list.html"><i></i><span>Daftar Mahasiswa</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-add-patient.html"><i></i><span>Tambah Mahasiswa</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-patient-profile.html"><i></i><span>Profil Mahasiswa</span></a>
                        </li>
                    </ul>
                </li>

                <li class=" nav-item"><a href="#"><i class="la la-users"></i><span class="menu-title" data-i18n="Patients">Test</span></a>
                    <ul class="menu-content">
                        <li><a class="menu-item" href="hospital-patients-list.html"><i></i><span>Kepribadian/MBTI</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-add-patient.html"><i></i><span>Tingkat Masalah</span></a>
                        </li>
                    </ul>
                </li>

                <li class=" nav-item"><a href="hospital-payment-reports.html"><i class="la la-bar-chart"></i><span class="menu-title" data-i18n="Report">Laporan</span></a>
                    <ul class="menu-content">
                        <li><a class="menu-item" href="hospital-patients-list.html"><i></i><span>Rekam Medis</span></a>
                        </li>
                        <li><a class="menu-item" href="hospital-add-patient.html"><i></i><span>Rekap Bulanan</span></a>
                        </li>
                    </ul>
                </li>

        
                <li class=" navigation-header"><span data-i18n="Apps">Apps</span><i class="la la-ellipsis-h" data-toggle="tooltip" data-placement="right" data-original-title="Apps"></i>
                </li>
                <li class=" nav-item"><a href="full-calender-basic.html"><i class="la la-calendar"></i><span class="menu-title" data-i18n="Calendar">Calendar</span></a>
                </li>
                <li class=" nav-item"><a href="app-email.html"><i class="la la-envelope"></i><span class="menu-title" data-i18n="Inbox">Inbox</span></a>
                </li>
                <li class=" nav-item"><a href="app-chat.html"><i class="la la-comments"></i><span class="menu-title" data-i18n="Chat">Chat</span></a>
                </li>
               
              
            </ul>
        </div>
    </div>